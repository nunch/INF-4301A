...
#define VISITOR_HH value
	...
	class Boolean : public Visitor<int>
	{
	public:
		Boolean(){
		};
		int visitBin(const Bin& exp) {
			return exp.accept(calcu);
		}

		int visitNum(const Num& exp) {
			return exp.val_;;
		}
		int visitIf(const IfExp& exp){
			return 0;
		}
		int visitVar(const Var& exp){
			return 0;
		}
		int visitShowVar(const ShowVar& exp){
			return 0;
		}
		int visitStringExp(const StringExp& exp){
			return 0;
		}
		int visitAco(const Aco& exp){
			return 0;
		}
		int visitFor(const ForExp& exp){
			return 0;
		...
		int visitAss(const Assignment& exp){
			return 0;
		}
		int visitNull(const Null& exp){
			return 0;
		}
		int visitLet(const LetExp& exp){
			return 0;
